...
#define ANOTHER_KEY_PRESSED (!IS_KEYPOS_SAME(prv_keypos, record->event.key))
  ...
  switch(mode){
    case DLT_MODE_SHORTPRESS_AND_KEYPRESS_DETECTED:
      ...
      a2 = action_for_key(0,(keypos_t)prv_keypos);
      action.code = a2.code;
    ...
    case DLT_MODE_LONGPRESS_AND_KEYPRESS_DETECTED:
      ...
      register_code(prv_keycode );
      unregister_code(prv_keycode );
      break;
    case DLT_MODE_LT:
...
bool inline process_action_delayed_lt(uint16_t keycode, keyrecord_t *record){
  ...
  if(dlt_on){
    ...
    //action_t a2 = action_for_key(0, record->event.key);
  ...
  switch(keycode&(~0xff)){
	...
	dlt_toggler = record->event.key;
        prv_keycode = keycode;//used to check is new key is pressed
    ...
    default:
      ...
      //
	//like with DLT keypress, upper 8bits are lost, refetch.
        prv_keycode = keymap_key_to_keycode(dlt_layer_to_toggle, record->event.key);
        return false;
      }else{//key release
